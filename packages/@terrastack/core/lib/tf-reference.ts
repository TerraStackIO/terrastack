import { IReferencableResource } from "./resource-object"
import {    
  snakeCase
} from "./_util";
/**
...
export class TfReference {
   ...
   */
  public static for(target: IReferencableResource, attribute: string, value: any) {
    if (value) {
      return value;
    } else {
      return `\$\{${target.logicalId()}.${snakeCase(attribute)}\}`
    }
  }

  // /**
  //  * Return a TfReference that references a pseudo referencd
  //  */
  // public static forPseudo(pseudoName: string, scope: Construct) {
  //   return TfReference.singletonReference(scope, `Pseudo:${pseudoName}`, () => {
  //     const cfnIntrinsic = { Ref: pseudoName };
  //     return new TfReference(cfnIntrinsic, pseudoName, scope);
  //   });
  // }

  // /**
  //  * Static table where we keep singleton TfReference instances
  //  */
  // private static referenceTable = new Map<Construct, Map<string, TfReference>>();

  // /**
  //  * Get or create the table
  //  */
  // private static singletonReference(target: Construct, attribKey: string, fresh: () => TfReference) {
  //   let attribs = TfReference.referenceTable.get(target);
  //   if (!attribs) {
  //     attribs = new Map();
  //     TfReference.referenceTable.set(target, attribs);
  //   }
